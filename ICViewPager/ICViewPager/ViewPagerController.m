...
- (NSInteger)initialIndex
{
    if (!_initialIndex)
    {
        ...
        self.initialIndex = (NSInteger)roundf(value);
    }
...
- (void)defaultSetup
{
    ...
    // Select starting tab
    NSUInteger index = self.initialIndex;
    if (self.initialIndex < 0 || self.initialIndex >= self.tabCount)
    {
        // If the initial index is invalid (i.e. too large or negative, just show the first tab)
        index = 0;
    }
    [self selectTabAtIndex:index didSwipe:NO];
